...
class CustomTokenCreateView(TokenCreateView):
    def _action(self, serializer):
        ...
        if user.is_locked_out:
            return Response(
                {
                    "error": f"Account is locked dut to multiple failed login attempts"
                    f"try again after {settings.LOCKEOUT_DURATION.total_seconds() // 60} minutes"
                },
...
class OTPVerifyView(APIView):
    ...
    def post(self, request):
        ...
        response = Response(
            {
                "success": "Login successfull. Now add your profile information, "
                "so that we can create an account for you"
            },
            status=status.HTTP_200_OK,
        )
        set_auth_cookies(response, access_token, refresh_token)
        logger.info(f"Successfull login with OTP: {user.email}")
