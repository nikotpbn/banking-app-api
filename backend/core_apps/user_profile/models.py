...
class Profile(TimeStampedModel):
    ...
    date_of_birth = models.DateField(
        _("Date of Birth"),
        default=settings.DEAFULT_BIRTHDATE,
    )
    ...
    identification_type = models.CharField(
        ...
        default=IdentificationType.DRIVER_LICENSE,
    )
    ...
    def clean(self) -> None:
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_issue_date:
                raise ValidationError("ID expiry date must come after issue date")
...
class NextOfKin(TimeStampedModel):
    ...
    relationship = models.CharField(_("Relationshopip"), max_length=50)
